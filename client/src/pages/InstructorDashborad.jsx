...
const categoryStyle = {
  ...
  Business:    { bg: "bg-[#FEF4E4]", text: "text-[#B07A1A]", icon: "ti-briefcase" },
  Marketing:   { bg: "bg-[#E6F0FB]", text: "text-[#2A6CB5]", icon: "ti-speakerphone" },
...
const emptyForm = { title: "", description: "", category: "Programming", price: 0 };
...
function Toast({ msg, type, onClose }) {
  ...
  const base = "fixed bottom-6 right-6 z-[100] flex items-center gap-2.5 px-4 py-3 rounded-[12px] shadow-lg text-[13.5px] font-medium border";
  const styles = type === "success"
    ? `${base} bg-[#E8F4ED] border-[#B6D9C5] text-[#2F7048]`
    : `${base} bg-red-50 border-red-200 text-red-600`;
  return (
    <div className={styles}>
      <i className={`ti ${type === "success" ? "ti-circle-check" : "ti-alert-circle"} text-[17px]`} />
      {msg}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div className="bg-white rounded-2xl border border-[#EAE8E3] w-full max-w-sm p-6 shadow-xl">
        ...
        <h3 className="text-[15px] font-semibold text-[#1A1A1A] text-center mb-1">Delete Course?</h3>
        <p className="text-[13px] text-[#888] text-center mb-5">
          "<span className="font-medium text-[#555]">{course?.title}</span>" will be permanently removed.
        </p>
...
function CourseForm({ initial, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setFormErr("");
    if (!form.title.trim()) { setFormErr("Course title is required."); return; }
  ...
  const inp = "w-full px-3.5 py-2.5 bg-[#FAFAF9] border border-[#E4E2DC] rounded-[10px] text-[13.5px] text-[#1A1A1A] placeholder-[#BDBAB4] focus:outline-none focus:border-[#3B8C5A] focus:bg-white transition";
  ...
  return (
    <div className="bg-white border border-[#EAE8E3] rounded-2xl p-6">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={onClose} className="w-8 h-8 bg-[#F7F5F2] hover:bg-[#EEE] border border-[#EAE8E3] rounded-[8px] flex items-center justify-center text-[#888] transition">
      ...
      {formErr && (
        <div className="mb-4 px-3.5 py-3 bg-red-50 border border-red-200 rounded-[10px] text-[12.5px] text-red-600 flex items-center gap-2">
          <i className="ti ti-alert-circle text-[14px]" />{formErr}
        </div>
      )}

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-[12.5px] font-medium text-[#444] mb-1.5">Course Title *</label>
          <input name="title" value={form.title} onChange={handleChange}
            placeholder="e.g. Complete Python Bootcamp" className={inp} />
        </div>
        <div>
          <label className="block text-[12.5px] font-medium text-[#444] mb-1.5">Description *</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[12.5px] font-medium text-[#444] mb-1.5">Category</label>
          ...
          <div>
            <label className="block text-[12.5px] font-medium text-[#444] mb-1.5">Price (USD)</label>
            <div className="relative">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#AAA] text-[13px]">$</span>
        ...
        <div className="flex items-center gap-3 p-3 bg-[#F7F5F2] rounded-[10px] border border-[#EAE8E3]">
          {(() => {
            ...
            return (
              <>
                <div className={`w-9 h-9 ${s.bg} rounded-[8px] flex items-center justify-center flex-shrink-0`}>
                  <i className={`ti ${s.icon} ${s.text} text-[17px]`} />
                </div>
                <div className="min-w-0">
                  <p className="text-[12.5px] font-medium text-[#1A1A1A] truncate">{form.title || "Course Title Preview"}</p>
                  <p className={`text-[11px] font-medium ${s.text}`}>{form.category} · {form.price === 0 ? "Free" : `$${form.price}`}</p>
                </div>
              </>
            );
          })()}
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button type="submit" disabled={saving}
            ...
            {saving
              ...
              : <><i className={`ti ${isEdit ? "ti-check" : "ti-circle-plus"} text-[15px]`} />{isEdit ? "Save Changes" : "Create Course"}</>}
          </button>
        </div>
      </form>
    </div>
  ...
  return (
    <div className="bg-white border border-[#EAE8E3] rounded-xl p-4 flex items-center gap-3.5 hover:border-[#D0D0C8] transition">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[14px] font-medium text-[#1A1A1A] truncate">{course.title}</p>
        <p className="text-[12px] text-[#AAA] mt-0.5">
          <span className={`${s.text} font-medium`}>{course.category}</span>
          {" · "}
          {course.lectures?.length ?? 0} lectures
          {" · "}
          <span className="text-[#3B8C5A] font-medium">{course.price === 0 ? "Free" : `$${course.price}`}</span>
        </p>
      </div>
      <div className="flex items-center gap-2 flex-shrink-0">
        <button onClick={() => onEdit(course)}
          className="w-8 h-8 bg-[#F7F5F2] hover:bg-[#E8F4ED] border border-[#EAE8E3] rounded-[8px] flex items-center justify-center transition" title="Edit">
          <i className="ti ti-pencil text-[#3B8C5A] text-[14px]" />
        </button>
        <button onClick={() => onDelete(course)}
          className="w-8 h-8 bg-[#F7F5F2] hover:bg-red-50 border border-[#EAE8E3] rounded-[8px] flex items-center justify-center transition" title="Delete">
    ...
    <aside className="w-[220px] bg-white border-r border-[#EAE8E3] flex flex-col px-4 py-6 fixed top-0 left-0 h-full z-30">
      ...
      <div className="mt-auto space-y-2">
        <div className="flex items-center gap-2.5 px-2.5 py-2">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-medium text-[#1A1A1A] truncate">{user?.email || "Instructor"}</p>
  ...
  const renderContent = () => {
    ...
    if (active === "Create Course") {
      return (
        <CourseForm
          initial={editTarget}
          onClose={() => { setActive("My Courses"); setEditTarget(null); }}
          onSave={handleSave}
        />
      );
    }
    ...
    if (active === "Students") {
      return (
        <div className="bg-white border border-[#EAE8E3] rounded-xl p-10 text-center">
          ...
          <p className="text-[12.5px] text-[#AAA] mt-1">Student management coming soon</p>
        </div>
      );
    }
      ...
      <>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-white border border-[#EAE8E3] rounded-xl p-4">
            ...
            <p className="text-[12px] text-[#3B8C5A] mt-1">Published on platform</p>
          </div>
          <div className="bg-white border border-[#EAE8E3] rounded-xl p-4">
          ...
          <div className="bg-white border border-[#EAE8E3] rounded-xl p-4">
            ...
            <p className="text-[22px] font-semibold text-[#1A1A1A]">{loading ? "—" : courses.length - freeCount}</p>
        ...
        <div className="flex justify-between items-center mb-3.5">
          <h2 className="text-[15px] font-semibold text-[#1A1A1A]">
            All Courses
            {!loading && <span className="ml-2 text-[12px] font-normal text-[#AAA]">({courses.length})</span>}
          </h2>
        </div>
        ...
        {apiError && (
          <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 rounded-[10px] flex items-center gap-2">
            ...
            <button onClick={fetchCourses} className="ml-auto text-[12px] text-red-500 underline">Retry</button>
          </div>
        )}

        {loading && <div className="flex flex-col gap-2.5">{[1, 2, 3].map((i) => <SkeletonRow key={i} />)}</div>}
        ...
        {!loading && !apiError && courses.length === 0 && (
          <div className="bg-white border border-[#EAE8E3] rounded-xl p-10 text-center">
            ...
            <p className="text-[12.5px] text-[#AAA] mt-1 mb-4">Create your first course to get started</p>
        ...
        {!loading && !apiError && courses.length > 0 && (
          <div className="flex flex-col gap-2.5">
            {courses.map((c) => (
              <CourseRow key={c._id} course={c} onEdit={handleEdit} onDelete={setDeleteTarget} />
            ))}
          </div>
        )}
      </>
        ...
        <div className="flex justify-between items-start mb-7">
          <div>
            <h1 className="text-[20px] font-semibold text-[#1A1A1A]">
              {getGreeting()}, {user?.email ? user.email.split("@")[0] : "Instructor"}
            </h1>
            <p className="text-[13px] text-[#999] mt-1">
              {active === "Create Course"
                ? editTarget ? "Edit your course details" : "Create a new course for students"
                : "Manage your courses from here"}
            </p>
          </div>
          {active !== "Create Course" && (
            <button onClick={() => handleSetActive("Create Course")}
